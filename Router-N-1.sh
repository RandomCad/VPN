...
if ! $(ip a | grep -q "enp0s8"); then
	echo "$Red No enp0s8 network interface detected. This interface while be used to conect to the rest of the world. Pleas set it up. $White"
  return 1
fi

#testing enp0s9 it shouldn't exist
if ! $(ip a | grep -q "enp0s9"); then
	echo "$Red enp0s9 network interface detected. This interface should not be used. Disconect it. $White"
  return 1
fi
...
sudo ./Router-N-1-Netplan-Conf.sh
...
ping -I enp0s9 -c 100 -i 0.002 8.8.8.8 | tee zwi
if ! cat zwi | grep -q "received, .% packet" ; then
