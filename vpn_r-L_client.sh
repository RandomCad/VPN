...
# diese Datei muss mit Superuser-Lechten ausgeführt werden
...
if [[ ! -f "./src/vpn_r-L_bridging.sh" ]]; then
    err "./src/vpn_r-L_bridging.sh ist nicht vorhanden! Diese ist für das Skript essentiell!"
fi 
if [[ ! -f "./src/r-L.conf" ]]; then
    if [[ ! -f "/etc/openvpn/r-L.conf" ]]; then
        err "./src/r-L.conf ist nicht vorhanden! Diese ist für das Skript essentiell!"
    else 
        warn "./src/r-L.conf ist nicht vorhanden! Jedoch existiert bereits eine OpenVPN Konfiguration /etc/openvpn/r-L.conf"
    fi 
fi 
